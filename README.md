...
|---|---|---|
| Чат поддержки | http://93.77.186.72 | Сотрудник, у которого возникла проблема |
| Рабочее место оператора | http://93.77.186.72/admin | Оператор первой линии поддержки |
...
**Чат поддержки, адрес `/`.** Главная страница: человек выбирает,
...
**Рабочее место оператора, адрес `/admin`.** Очередь обращений, разбор, проверка, трасса шагов
...
Дообучить модель на диалогах операторов Сбера с клиентами нельзя: они закрыты
и содержат персональные данные. Поэтому система учится на собственных данных,
которые порождает с первого дня работы.

Каждый раз, когда оператор меняет приоритет, категорию, команду или переписывает
письмо, система запоминает пару «что предложил агент» и «на что это исправил
человек» вместе с выжимкой обращения.

Эти правки работают двумя способами.

**Сразу, без обучения.** Правки подмешиваются в промпт разбора как примеры,
переписанные оператором письма — в промпт автора ответа как образец тона.
Это действует с первой же правки: модель видит, что в похожем случае человек
поднял приоритет, и в следующий раз ставит его сам.

**Позже, для настоящего дообучения.** `GET /api/training/dataset.jsonl` отдаёт
накопленное в формате Yandex Cloud: пара запрос-ответ на строку. Когда правок
наберётся несколько тысяч, из них получится обучающая выборка.
`GET /api/training/corrections` показывает, что система уже узнала, и сколько
осталось до порога.

Отклонение предложения агента тоже записывается: это сигнал, что решение
было неверным целиком, а не в одном поле.
...
Откройте `http://localhost:5173` — чат поддержки.
Рабочее место оператора открывается по адресу `/admin`.
