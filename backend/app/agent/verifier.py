...
# Вежливость, сочувствие и служебные строки утверждениями не являются:
# проверять в них нечего. Фраза «без VPN работать нечем» ничего не сообщает
# о мире сверх того, что человек уже написал сам, и требовать к ней ссылку
# на базу знаний бессмысленно.
BOILERPLATE = re.compile(
    r"^(здравствуйте|добрый день|спасибо|с уважением|служба технической поддержки|"
    r"если .{0,80}(не помог|останется|возникн|потребу|не получ)|будем рады|хорошего дня|"
    ...
    r"пон(имаю|ятно)|это действительно|давайте (разбер|почин|посмотр)|"
    r"неприятн|понимаю, что|сейчас разбер|уже понял|мы (вас )?(прочитали|поняли)|"
    r"напишите.{0,40}(разбер|поможем)|подскажите|уточните)",
    re.I,
)
